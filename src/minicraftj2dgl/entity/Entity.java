...
public abstract class Entity implements Serializable {
    ...
    protected boolean move2(int xa, int ya) {
        ...
        // Call touchedBy for all entities except itself.
        isInside.stream().filter((e) -> !(e == this)).forEach((e) -> {
            e.touchedBy(this);
        });

        isInside.removeAll(wasInside);

        // Check if any entity satisfies blocks(this), skipping itself in the comparison.
        if (!isInside.stream().filter((e) -> !(e == this)).noneMatch((e) -> (e.blocks(this)))) {
            return false;
        }
