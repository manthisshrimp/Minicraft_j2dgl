...
import minicraftj2dgl.entity.Spark;
...
public class AirWizard extends Mob {
    ...
    public void tick() {
        ...
        if (attackTime > 0) {
            attackTime--;
            double dirWhitIsThis = attackTime * 0.25 * (attackTime % 2 * 2 - 1);
            double speed = (0.7) + attackType * 0.2;
            level.add(new Spark(this, Math.cos(dirWhitIsThis) * speed, Math.sin(dirWhitIsThis) * speed));
            return;
        }
